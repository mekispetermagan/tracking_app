...
class CourseVisitReportViewer extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        ...
        if (report.mentors.isEmpty)
          const Text('No assigned mentor was recorded as present.')
        else
          ...report.mentors.map(
            (mentor) => Padding(
        ...
        const SizedBox(height: 16),
        _sectionTitle(context, 'Students present (${report.students.length})'),
        const SizedBox(height: 8),
        if (report.students.isEmpty)
          const Text('No enrolled student was recorded as present.')
        else
          Wrap(
            spacing: 8,
            runSpacing: 8,
                ...
                subtitle: Text(
                  [
                    ...
                    if (action.targetDate != null)
                      'Target: ${_formatDate(action.targetDate!)}',
                  ].join('\n'),
                ),
              ),
            ),
